...
int main(int argc, char **argv)
{
        ...
        //(can divide/assign permutations by methods other than bitwise
        ...
        for (counter = 0; counter < maxSpace; counter++) {
                ...
                // Key match checking
                // Hint - If key is found(decrypted string matches) return value
                // back to parent
                if (strncmp(plaintext, plain_in, plain_length) == 0) {
                        printf("\nOK: enc/dec ok for \"%s\"\n", plaintext);
                        printf("Key No.:%lu:", counter);
                ...
                free(plaintext);
                /*
                     Hint - In child processes make sure to release allocated
                   memory and if key found return key to parent. Lastly have
